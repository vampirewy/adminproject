  ...
  <header class="header">
    <div class="left right">
      ...
      <el-checkbox-group v-model="statusList" @change="seletItem">
        <el-checkbox v-for="(item,index) in checkBoxLists" :label="item.name" :key="index">{{item.name}}</el-checkbox>
      </el-checkbox-group>
    ...
    <div class="middle">
      <el-select v-model="value" @blur="changeArea()">
        <el-option v-for="item in allAreaName" :key="item.traName" :label="item.traName" :value="item.traId"></el-option>
      </el-select>
    </div>
    <!-- 导购列表、专题列表 -->
    ...
    <!-- 弹窗列表 -->
    <div class="right" v-if="!showArea">
      <el-autocomplete v-model="popName" :fetch-suggestions="querySearchAsync" placeholder="请输入弹窗名称" @select="selecthPopName"></el-autocomplete>
...
export default {
  ...
  methods: {
    selectAll(val) {
      ...
      val ? this.checkBoxLists.forEach(el => {
